	...
	return (
		<div className='bg-white p-6 rounded-lg flex flex-col md:flex-row gap-6 w-5/6 md:w-[721px] m-auto'>
			<div className='md:w-[246px] w-1/3'>
				<img className='rounded-lg h-full object-cover' src='/login.png' alt='Login Image' />
			</div>
			<div className='md:w-[403px] w-2/3'>
				...
				<div className='mt-6'>
					<div>
						...
						<label className="relative block">
							<span className="absolute inset-y-0 top-2 left-0 flex pointer-events-auto items-center pl-2">
								{/* <IoSearch className='w-[18px] h-[18px] relative' /> */}
					...
					<div className='mt-[18px]'>
						...
						<div className="relative  rounded-md shadow-sm">
							<input type='password' id="password" className='w-full mt-[10px] outline-none rounded-md border-2 pl-4 ' placeholder='Please Input here.' onChange={text =>{ setPassword(text.target.value)}}/>
							<div className="absolute inset-y-0 right-0 flex pt-3 pr-5 items-center ">
								<button  onClick={()=>{
											...
											if (x.type === "password") {
												...
												x.type = "password";
											}}}><IoEyeOutline className='w-5 h-5 justify-center items-center flex'/>
								</button>
							</div>
						</div>
					</div>
				...
				<div className='my-6'>
					<button className='w-full py-2 rounded-lg border-2 flex justify-center text-[14px]' onClick={() => googleLogin()}>
